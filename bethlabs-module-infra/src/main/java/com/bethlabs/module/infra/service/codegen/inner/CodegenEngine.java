package com.bethlabs.module.infra.service.codegen.inner;
...
import com.bethlabs.framework.apilog.core.enums.OperateTypeEnum;
...
import com.bethlabs.framework.common.util.date.DateUtils;
import com.bethlabs.framework.common.util.date.LocalDateTimeUtils;
import com.bethlabs.framework.common.util.object.BeanUtils;
import com.bethlabs.framework.common.util.object.ObjectUtils;
import com.bethlabs.framework.common.util.string.StrUtils;
import com.bethlabs.framework.excel.core.annotations.DictFormat;
import com.bethlabs.framework.excel.core.convert.DictConvert;
...
import com.bethlabs.module.infra.dal.dataobject.codegen.CodegenColumnDO;
import com.bethlabs.module.infra.dal.dataobject.codegen.CodegenTableDO;
import com.bethlabs.module.infra.enums.codegen.CodegenFrontTypeEnum;
import com.bethlabs.module.infra.enums.codegen.CodegenSceneEnum;
import com.bethlabs.module.infra.enums.codegen.CodegenTemplateTypeEnum;
import com.bethlabs.module.infra.enums.codegen.CodegenVOTypeEnum;
import com.bethlabs.module.infra.framework.codegen.config.CodegenProperties;
...
public class CodegenEngine {
    ...
    /**
     * 是否为 bethlabs-cloud 项目，用于解决 Boot 和 Cloud 的 api 模块兼容性问题
     *
     * true  - 需要有 bethlabs-module-xxx-api 模块
    ...
    public CodegenEngine() {
        ...
        this.cloudEnable = ClassUtils.isPresent("com.bethlabs.module.infra.framework.rpc.config.RpcConfiguration",
    ...
    private Map<String, String> getTemplates(Integer frontType) {
        ...
        if (Boolean.FALSE.equals(cloudEnable)) {
            SERVER_TEMPLATES.forEach((templatePath, filePath) -> {
                filePath = StrUtil.replace(filePath, "/bethlabs-module-${table.moduleName}-api", "");
                filePath = StrUtil.replace(filePath, "/bethlabs-module-${table.moduleName}-server", "");
    ...
    private static String javaModuleFilePath(String path, String module, String src) {
        return "bethlabs-module-${table.moduleName}/" + // 顶级模块
                "bethlabs-module-${table.moduleName}-" + module + "/" + // 子模块
                "src/" + src + "/java/${basePackage}/module/${table.moduleName}/" + path + ".java";
    }

    private static String mapperXmlFilePath() {
        return "bethlabs-module-${table.moduleName}/" + // 顶级模块
                "bethlabs-module-${table.moduleName}-server/" + // 子模块
    ...
    private static String vueFilePath(String path) {
        return "bethlabs-ui-${sceneEnum.basePackage}-vue2/" + // 顶级目录
                "src/" + path;
    }
    ...
    private static String vue3FilePath(String path) {
        return "bethlabs-ui-${sceneEnum.basePackage}-vue3/" + // 顶级目录
                "src/" + path;
    }
