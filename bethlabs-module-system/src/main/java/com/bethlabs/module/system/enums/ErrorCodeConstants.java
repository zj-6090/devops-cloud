package com.bethlabs.module.system.enums;

import com.bethlabs.framework.common.exception.ErrorCode;
...
public interface ErrorCodeConstants {
    ...
    // ========== 分组模块 1-002-004-000 ==========
    ErrorCode DEPT_NAME_DUPLICATE = new ErrorCode(1_002_004_000, "已经存在该名字的分组");
    ErrorCode DEPT_PARENT_NOT_EXITS = new ErrorCode(1_002_004_001,"父级分组不存在");
    ErrorCode DEPT_NOT_FOUND = new ErrorCode(1_002_004_002, "当前分组不存在");
    ErrorCode DEPT_EXITS_CHILDREN = new ErrorCode(1_002_004_003, "存在子分组，无法删除");
    ErrorCode DEPT_PARENT_ERROR = new ErrorCode(1_002_004_004, "不能设置自己为父分组");
    ErrorCode DEPT_NOT_ENABLE = new ErrorCode(1_002_004_006, "分组({})不处于开启状态，不允许选择");
    ErrorCode DEPT_PARENT_IS_CHILD = new ErrorCode(1_002_004_007, "不能设置自己的子分组为父分组");
    ...
    // ========== 产品 ==========
    ErrorCode PRODUCT_NOT_EXISTS = new ErrorCode(1, "产品不存在");

    ErrorCode PRODUCT_CODE_EXISTS = new ErrorCode(1, "产品编码已存在，不能重复");

    ErrorCode PRODUCT_NAME_EXISTS = new ErrorCode(1, "产品名称已存在，不能重复");

    ErrorCode PRODUCT_DEV_EXISTS = new ErrorCode(1, "该产品下有设备接口存在，不能删除");

    ErrorCode PRODUCT_ALARM_TYPE_EXISTS = new ErrorCode(1, "该产品下有告警类型存在，不能删除");

    // ========== 设备接口 ==========

    ErrorCode PRODUCT_DEV_NOT_EXISTS = new ErrorCode(1, "产品设备接口不存在");

    ErrorCode PRODUCT_DEV_NAME_EXISTS = new ErrorCode(1, "设备接口名称已存在，不能重复");

    // ========== 产品-报警类型 ==========
    ErrorCode ALARM_TYPE_NOT_EXISTS = new ErrorCode(1, "报警类型不存在");

    ErrorCode ALARM_TYPE_EXISTS = new ErrorCode(1, "报警类型已存在，不能重复");

    ErrorCode DATA_BUCKUPS_NOT_EXISTS = new ErrorCode(1, "数据备份文件不存在");

    ErrorCode CONFIG_NOT_EXISTS = new ErrorCode(1, "系统配置不存在");

    ErrorCode PAGE_MAINTENANCE_NOT_EXISTS = new ErrorCode(1, "页面维护不存在");


    ErrorCode PAGE_MAINTENANCE_ENABLE_FAIL = new ErrorCode(1, "启用失败，已启用相同系列和版本号的页面维护");
