...
public class PermissionAssignRoleDataScopeReqVO {
    ...
    @Schema(description = "分组编号列表，只有范围类型为 DEPT_CUSTOM 时，该字段才需要", example = "1,3,5")
    private Set<Long> dataScopeDeptIds = Collections.emptySet(); // 兜底

    @Schema(description = "项目编号列表，只有范围类型为 DEPT_CUSTOM 时，该字段才需要", example = "1,3,5")
    private Set<Long> dataScopeProjectIds = Collections.emptySet(); // 兜底
